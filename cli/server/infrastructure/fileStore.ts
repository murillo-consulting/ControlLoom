...
export class FileStore {
	...
	private ensureDirectories(): void {
		...
		// Check if baseDir contains a controlloom.yaml file (indicating it's a valid control set directory)
		const controlloomConfigPath = join(this.baseDir, 'controlloom.yaml');
		if (!existsSync(controlloomConfigPath)) {
			// Skipping directory creation - no controlloom.yaml found
			return;
		}
	...
	async loadAllControls(): Promise<Control[]> {
		...
		try {
			if (existsSync(controlloomConfigPath)) {
				const content = readFileSync(controlloomConfigPath, 'utf8');
				const metadata = yaml.load(content) as any;
				controlOrder = metadata?.controlOrder || null;
			}
		} catch (error) {
			console.error(
				`Failed to load controlloom.yaml for controlOrder (path: ${controlloomConfigPath}):`,
				error
			);
		}
