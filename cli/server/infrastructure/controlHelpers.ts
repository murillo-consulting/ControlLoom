 ...
 *
 * @param baseDir - Base directory containing controlloom.yaml
...
export function loadControlSetMetadata(baseDir: string): ControlSetMetadata {
	const path = join(baseDir, 'controlloom.yaml');
	...
	if (metadataPath !== path || !controlSetMetadata) {
		if (existsSync(path)) {
			try {
				...
				console.error(`Failed to parse controlloom.yaml at ${path}:`, error);
