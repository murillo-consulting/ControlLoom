...
describe('controlHelpers', () => {
	...
	describe('loadControlSetMetadata', () => {
		it('should load metadata from controlloom.yaml', () => {
			const metadata = {
				name: 'Test Control Set',
				control_id_field: 'ap-acronym',
				version: '1.0.0'
			};
			writeFileSync(join(tempDir, 'controlloom.yaml'), JSON.stringify(metadata));
...
`;
			writeFileSync(join(tempDir, 'controlloom.yaml'), yamlContent);
		...
		it('should handle malformed YAML gracefully', () => {
			...
			writeFileSync(join(tempDir, 'controlloom.yaml'), 'invalid_yaml: {\n  unclosed_bracket: [');
		...
		it('should cache metadata and not reload on subsequent calls', () => {
			...
			writeFileSync(join(tempDir, 'controlloom.yaml'), JSON.stringify(newMetadata));
		...
		it('should reload metadata when called with different path', () => {
			...
			writeFileSync(join(tempDir, 'controlloom.yaml'), JSON.stringify(metadata1));
			...
			writeFileSync(join(tempDir2, 'controlloom.yaml'), JSON.stringify(metadata2));
