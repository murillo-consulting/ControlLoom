...
describe('spreadsheetRoutes', () => {
	...
	describe('scanControlSets', () => {
		it('should return empty control sets when no controlloom.yaml files found', async () => {
			mockGlob.mockResolvedValue([]);

			const result = await scanControlSets();

			expect(result).toEqual({ controlSets: [] });
			expect(mockGlob).toHaveBeenCalledWith('**/controlloom.yaml', {
		...
		it('should scan and return valid control sets', async () => {
			const mockFiles = ['subfolder1/controlloom.yaml', 'subfolder2/controlloom.yaml'];
			...
			expect(result.controlSets[0]).toEqual({
				...
				description: 'First test control set',
				controlCount: 10,
				file: 'subfolder1/controlloom.yaml'
			});
			expect(result.controlSets[1]).toEqual({
				...
				description: 'Second test control set',
				controlCount: 15,
				file: 'subfolder2/controlloom.yaml'
			});
		});

		it('should skip default/placeholder control sets', async () => {
			const mockFiles = ['default/controlloom.yaml', 'valid/controlloom.yaml'];
			mockGlob.mockResolvedValue(mockFiles);
			...
			expect(result.controlSets[0]).toEqual({
				...
				description: 'Valid control set',
				controlCount: 5,
				file: 'valid/controlloom.yaml'
			});
		});

		it('should handle invalid yaml files gracefully', async () => {
			const mockFiles = ['invalid/controlloom.yaml'];
			mockGlob.mockResolvedValue(mockFiles);
			...
			expect(result.controlSets[0]).toEqual({
				path: 'invalid',
				name: 'Invalid controlloom.yaml',
				description: 'Could not parse file',
				controlCount: 0,
				file: 'invalid/controlloom.yaml'
			});
		});

		it('should handle files in base directory', async () => {
			const mockFiles = ['controlloom.yaml'];
			mockGlob.mockResolvedValue(mockFiles);
			...
			expect(result.controlSets[0]).toEqual({
				...
				description: 'Control set in base directory',
				controlCount: 8,
				file: 'controlloom.yaml'
			});
		});

		it('should provide default values for missing fields', async () => {
			const mockFiles = ['minimal/controlloom.yaml'];
			mockGlob.mockResolvedValue(mockFiles);
			...
			expect(result.controlSets[0]).toEqual({
				...
				description: '',
				controlCount: 0,
				file: 'minimal/controlloom.yaml'
			});
		});

		it('should handle file read errors gracefully', async () => {
			const mockFiles = ['error/controlloom.yaml'];
			mockGlob.mockResolvedValue(mockFiles);
		...
		it('should handle deeply nested control sets', async () => {
			const mockFiles = ['level1/level2/level3/controlloom.yaml'];
			mockGlob.mockResolvedValue(mockFiles);
			...
			expect(result.controlSets[0]).toEqual({
				...
				description: 'Control set in nested directory',
				controlCount: 3,
				file: 'level1/level2/level3/controlloom.yaml'
			});
		});

		it('should handle multiple control sets with mixed valid and invalid files', async () => {
			const mockFiles = [
				'valid1/controlloom.yaml',
				'invalid/controlloom.yaml',
				'default/controlloom.yaml',
				'valid2/controlloom.yaml'
			];
			mockGlob.mockResolvedValue(mockFiles);
	...
	describe('createOutputStructure', () => {
		...
		it('should create directory structure and files correctly', async () => {
			...
			expect(mockMkdirSync).toHaveBeenCalledWith(join('/test/control-sets', 'test-control-set'), {
				recursive: true
			});
			expect(mockMkdirSync).toHaveBeenCalledWith(
				join('/test/control-sets', 'test-control-set', 'controls', 'AC'),
				{
					recursive: true
				}
			);
			expect(mockWriteFileSync).toHaveBeenCalledWith(
				join('/test/control-sets', 'test-control-set', 'controlloom.yaml'),
				'mocked yaml content'
			);
		});
		...
		it('should handle justification fields correctly', async () => {
			...
			expect(mockWriteFileSync).toHaveBeenCalledWith(
				join('/test/control-sets', 'test-control-set', 'mappings', 'AC', 'AC-1-mappings.yaml'),
				'mocked yaml content'
			);
		});
