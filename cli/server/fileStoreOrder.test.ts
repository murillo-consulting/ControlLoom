...
describe('FileStore Order Preservation', () => {
	...
	beforeEach(async () => {
		tempDir = fs.mkdtempSync(path.join(tmpdir(), 'controlloom-filestore-test-'));
		...
		fs.writeFileSync(path.join(tempDir, 'controlloom.yaml'), yaml.dump(metadata));
	...
	it('should handle missing controlOrder gracefully', async () => {
		const metadataPath = path.join(tempDir, 'controlloom.yaml');
