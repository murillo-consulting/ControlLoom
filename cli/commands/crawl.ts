...
const closingBody = `\n\n---\n\n<sub>**Tip:** Customize your compliance reviews with <a href="https://github.com/murillo-consulting/ControlLoom.git" class="Link--inTextBlock" target="_blank" rel="noopener noreferrer">ControlLoom</a>.</sub>`;
export const CONTROLLOOM_SIGNATURE = '<!-- CONTROLLOOM_SIGNATURE:v1 -->';
...
/**
 * Extracts all @controlloomStart and @controlloomEnd blocks from the given content.
 *
...
export function extractMapBlocks(content: string): {
	...
	lines.forEach((line, idx) => {
		const start = line.match(/@controlloomStart\s+([a-f0-9-]+)/);
		const end = line.match(/@controlloomEnd\s+([a-f0-9-]+)/);
...
/**
 * Extract the content between @controlloomStart and @controlloomEnd
 *
 * @param lines The array of lines.
 * @param startLine The line number of @controlloomStart
 * @param endLine The line number of @controlloomEnd
...
/**
 * Check if a text contains ControlLoom annotations (@controlloomStart or @controlloomEnd).
 *
 * @param text The text content to check.
 * @returns True if the text contains ControlLoom annotations, false otherwise.
 */
export function containsControlLoomAnnotations(text: string): boolean {
	const lines = text.split('\n');
	return lines.some(
		(line) => line.includes('@controlloomStart') || line.includes('@controlloomEnd')
	);
}
...
export function createInitialCommentBody(filesCount: number): string {
	return (
		...
		`## ControlLoom Compliance Overview\n\n` +
		`Please review the changes to ensure they meet compliance standards.\n\n` +
		`### Reviewed Changes\n\n` +
		`ControlLoom reviewed ${filesCount} files changed that affect compliance.\n\n`
	);
}

/**
 * Check deleted files for ControlLoom annotations and generate warning content.
 */
export async function analyzeDeletedFiles(context: CrawlContext): Promise<{
	...
	for (const file of files) {
		if (file.status === 'removed') {
			try {
				...
				if (containsControlLoomAnnotations(oldText)) {
	...
	warningContent += `The following files contained compliance annotations (\`@controlloomStart\`/\`@controlloomEnd\`) and were deleted in this PR. This may affect compliance coverage:\n\n`;
...
export function generateRemovedBlocksContent(
	...
	let content = `\n\n**Compliance Warning: ControlLoom annotations were removed from \`${filename}\`**\n\n`;
...
export async function analyzeModifiedFiles(context: CrawlContext): Promise<{
	...
	for (const file of files) {
		...
		try {
			...
			// This filters out cases where new ControlLoom annotations are added but existing ones unchanged
			...
			if (hasActualComplianceChanges) {
				...
				if (newBlocks.length > oldBlocks.length) {
					console.log(
						`Skipping ${file.filename}: only new ControlLoom annotations added, no existing compliance content modified`
					);
				}
			}
		} catch (err) {
...
export function crawlCommand(): Command {
	return new Command()
		.command('crawl')
		.description(
			'Detect compliance-related changes between @controlloomStart and @controlloomEnd in PR files'
		)
		.addOption(
...
export async function deleteOldIssueComments({
	...
	while (true) {
		...
		for (const c of comments) {
			const hasSignature = (c.body ?? '').includes(CONTROLLOOM_SIGNATURE);
			if (hasSignature) {
...
export async function deleteOldReviewComments({
	...
	while (true) {
		...
		for (const rc of reviewComments) {
			const hasSignature = (rc.body ?? '').includes(CONTROLLOOM_SIGNATURE);
			if (hasSignature) {
...
export async function dismissOldReviews({
	...
	while (true) {
		...
		for (const r of reviews) {
			const hasSignature = (r.body ?? '').includes(CONTROLLOOM_SIGNATURE);
