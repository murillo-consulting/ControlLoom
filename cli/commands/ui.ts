...
export class UICommand {
	...
	static register(program: Command, parentDebugGetter: () => boolean): Command {
		return program
			...
			.description('Start the ControlLoom web interface (default)')
	...
	async run(options: UIOptions): Promise<void> {
		...
		// Check if controlloom.yaml exists in the directory
		const controlSetPath = join(dir, 'controlloom.yaml');
