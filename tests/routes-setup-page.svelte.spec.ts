...
async function gotoAndWaitMounted({ page }: { page: Page }): Promise<void> {
	...
	// Anchor on the ControlLoom logo (most stable element on this page)
	await page.waitForSelector('img[alt="ControlLoom"]', { timeout: 20000 });
...
test.describe('Setup Wizard', () => {
	test('default landing (no control sets)', async ({ page }) => {
		...
		await expect(page.locator('img[alt="ControlLoom"]')).toBeVisible();
		await expect(page.locator('#title')).toHaveText('ControlLoom');
