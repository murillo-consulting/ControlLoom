...
const OWNER = process.env.CONTROLLOOM_TEST_OWNER ?? '';
const REPO = process.env.CONTROLLOOM_TEST_REPO ?? '';
const PULL_NUMBER = Number(process.env.CONTROLLOOM_TEST_PULL_NUMBER ?? 0);
...
describe.skipIf(!OWNER || !REPO || !PULL_NUMBER)('crawl', () => {
  beforeAll(() => {
    ...
    try {
      ...
      command_output = execSync(
        `OWNER=${OWNER} REPO=${REPO} PULL_NUMBER=${PULL_NUMBER} GITHUB_TOKEN=${GITHUB_TOKEN} npx controlloom crawl --post-mode=comment`,
        {
          encoding: 'utf8',
          timeout: 60000
        }
  ...
  it("should console log the ControlLoom overview, table with lines changed, uuid and sha256sum when content changes between annotations", { timeout: 2 * 60 * 1000 }, () => {
    ...
    expect(command_output).toContain("## ControlLoom Compliance Overview");
  ...
  it('posts a PR comment with the ControlLoom overview and block metadata', { timeout: 2 * 60 * 1000 }, async () => {
    ...
    const relevantComments: IssueComment[] = (comments as IssueComment[]).filter((comment: IssueComment) => {
      ...
      return created >= testStartTime && comment.body.includes("## ControlLoom Compliance Overview");
    });
    ...
    for (const c of relevantComments) {
      ...
      expect(body).toContain("## ControlLoom Compliance Overview");
  ...
  it('should detect when @controlloomStart/@controlloomEnd annotations are DELETED from an existing file', { timeout: 2 * 60 * 1000 }, async () => {
  ...
  it('should detect when a whole file with annotations is DELETED', { timeout: 2 * 60 * 1000 }, async () => {    
    const { containsControlLoomAnnotations } = await import('../cli/commands/crawl.js');
    ...
    expect(containsControlLoomAnnotations(fileWithAnnotations)).toBe(true);
    expect(containsControlLoomAnnotations(fileWithoutAnnotations)).toBe(false);
    ...
    expect(containsControlLoomAnnotations(fileWithOnlyStart)).toBe(true);
    expect(containsControlLoomAnnotations(fileWithOnlyEnd)).toBe(true);
  });
  ...
  it('should NOT make any ControlLoom comments about the multiple-same-uuids.yaml file', { timeout: 2 * 60 * 1000 }, () => {
    expect(command_output).toContain("Skipping integration/test-files/multiple-same-uuids.yaml: only new ControlLoom annotations added, no existing compliance content modified");
  ...
  it('should NOT include multiple-same-uuids.yaml references in ControlLoom comments', { timeout: 2 * 60 * 1000 }, async () => {
    // Give GitHub a moment to persist comments
    await sleep(10);
